import torch
import torch.nn as nn
import torch.nn.functional as F
import math
from .utils import CfgNode


class SwiGLU(nn.Module):
    """SwiGLU 激活函数，用于 FFN"""
    def __init__(self, config):
        super(SwiGLU, self).__init__()
        hidden_dim = int(config.n_embd * 8 / 3)
        # 保证hidden_dim 是 multiple_of的倍数 方便硬件加速
        hidden_dim = ((hidden_dim + config.multiple_of - 1) // config.multiple_of) * config.multiple_of
        self.w_gate = nn.Linear(config.n_embd, hidden_dim, bias=False)
        self.w_up = nn.Linear(config.n_embd, hidden_dim, bias=False)
        self.w_down = nn.Linear(hidden_dim, config.n_embd, bias=False)

    def forward(self, x):
        return self.w_down(self.w_up(x) * F.silu(self.w_gate(x)))


class CausalSelfAttention(nn.Module):
    """因果自注意力层，带掩码防止看到未来信息"""
    def __init__(self, config):
        super(CausalSelfAttention, self).__init__()
        assert config.n_embd % config.n_head == 0
        self.c_attn = nn.Linear(config.n_embd, 3 * config.n_embd, bias=False)
        self.c_proj = nn.Linear(config.n_embd, config.n_embd, bias=False)

        self.register_buffer("mask",
                             torch.tril(torch.ones(config.block_size, config.block_size, dtype=torch.bool)
                                        .view(1, 1, config.block_size, config.block_size)))
        self.n_head = config.n_head
        self.n_embd = config.n_embd

    def forward(self, x):
        batchsize, seq_len, d_model = x.size()
        q, k, v = self.c_attn(x).chunk(3, dim=-1)
        q = q.view(batchsize, seq_len, self.n_head, d_model // self.n_head).transpose(1, 2)
        k = k.view(batchsize, seq_len, self.n_head, d_model // self.n_head).transpose(1, 2)
        v = v.view(batchsize, seq_len, self.n_head, d_model // self.n_head).transpose(1, 2)

        attn = (q @ k.transpose(-2, -1)) * (1.0 / math.sqrt(d_model // self.n_head))
        attn = attn.masked_fill(~self.mask[:, :, :seq_len, :seq_len], -1e9)
        attn = F.softmax(attn, dim=-1)

        y = torch.matmul(attn, v)
        y = y.transpose(1, 2).contiguous().view(batchsize, seq_len, d_model)
        y = self.c_proj(y)

        return y


class RMSNorm(nn.Module):
    """RMSNorm 归一化层"""
    def __init__(self, config):
        super(RMSNorm, self).__init__()
        self.gamma = nn.Parameter(torch.ones(config.n_embd))

    def forward(self, x):
        rms = torch.rsqrt(torch.pow(x, 2).mean(dim=-1, keepdim=True) + 1e-12)
        x_normed = x * rms * self.gamma
        return x_normed


class Block(nn.Module):
    """Transformer 解码器块：Attention + FFN + 残差连接"""
    def __init__(self, config):
        super(Block, self).__init__()
        self.norm1 = RMSNorm(config)
        self.attn = CausalSelfAttention(config)
        self.norm2 = RMSNorm(config)
        self.ffn = SwiGLU(config)

    def forward(self, x):
        _x = self.norm1(x)
        _x = self.attn(_x)
        x = _x + x
        _x = self.norm2(x)
        _x = self.ffn(_x)
        x = _x + x
        return x


class GPT(nn.Module):
    """GPT 语言模型"""
    
    @staticmethod
    def get_default_config():
        C = CfgNode()
        C.model_type = "gpt"
        C.n_layer = None
        C.n_head = None
        C.n_embd = None
        C.block_size = None
        C.vocab_size = None
        C.multiple_of = 256
        return C

    def __init__(self, config: CfgNode):
        super(GPT, self).__init__()

        assert config.vocab_size is not None, "必须提供 vocab_size"
        assert config.block_size is not None, "必须提供 block_size"
        self.block_size = config.block_size

        type_given = config.model_type is not None and config.model_type != "gpt"
        params_given = all([config.n_layer is not None, config.n_head is not None, config.n_embd is not None])
        assert type_given or params_given, "必须提供 model_type 或 (n_layer, n_head, n_embd)"

        if type_given:
            model_configs = {
                'openai-gpt': dict(n_layer=12, n_head=12, n_embd=768, multiple_of=256),
                'gpt2': dict(n_layer=12, n_head=12, n_embd=768, multiple_of=256),
                'gpt2-medium': dict(n_layer=24, n_head=16, n_embd=1024, multiple_of=256),
                'gpt2-large': dict(n_layer=36, n_head=20, n_embd=1280, multiple_of=256),
                'gpt2-xl': dict(n_layer=48, n_head=25, n_embd=1600, multiple_of=256),
                'gopher-44m': dict(n_layer=8, n_head=16, n_embd=512, multiple_of=256),
                'gpt-mini': dict(n_layer=6, n_head=6, n_embd=192, multiple_of=256),
                'gpt-micro': dict(n_layer=4, n_head=4, n_embd=128, multiple_of=256),
                'gpt-nano': dict(n_layer=3, n_head=3, n_embd=48, multiple_of=256),
            }
            if config.model_type in model_configs:
                config.merge_from_dict(model_configs[config.model_type])

        self.config = config
        
        self.transformer = nn.ModuleDict(dict(
            wte=nn.Embedding(config.vocab_size, config.n_embd),
            wpe=nn.Embedding(config.block_size, config.n_embd),
            h=nn.ModuleList([Block(config) for _ in range(config.n_layer)]),
            norm_f=RMSNorm(config),
        ))
        self.lm_head = nn.Linear(config.n_embd, config.vocab_size, bias=False)
        
        self.apply(self._init_weights)
        for name, param in self.transformer.named_parameters():
            if name.endswith('c_proj.weight'):
                torch.nn.init.normal_(param, mean=0, std=0.02 / math.sqrt(config.n_layer))

        n_params = sum(param.numel() for param in self.parameters())
        n_params_without_lm_head = sum(param.numel() for param in self.transformer.parameters())
        print(f"模型参数量: {n_params / 1e6:.2f}M")
        print(f"模型参数量(不含lm_head): {n_params_without_lm_head / 1e6:.2f}M")

    def _init_weights(self, m):
        if isinstance(m, nn.Linear):
            torch.nn.init.normal_(m.weight, 0, 0.02)
            if m.bias is not None:
                torch.nn.init.zeros_(m.bias)
        elif isinstance(m, nn.Embedding):
            torch.nn.init.normal_(m.weight, 0, 0.02)
        elif isinstance(m, RMSNorm):
            torch.nn.init.ones_(m.gamma)

    def configure_optimizers(self, train_config):
        """配置优化器，区分 weight decay"""
        decay = set()
        no_decay = set()
        whitelist_weight_modules = (torch.nn.Linear,)
        blacklist_weight_modules = (torch.nn.Embedding, )

        for mn, m in self.named_modules():
            for pn, p in m.named_parameters():
                full_param_name = f"{mn}.{pn}" if mn else pn
                if pn.endswith('bias'):
                    no_decay.add(full_param_name)
                elif pn.endswith('weight') and isinstance(m, whitelist_weight_modules):
                    decay.add(full_param_name)
                elif pn.endswith('weight') and isinstance(m, blacklist_weight_modules):
                    no_decay.add(full_param_name)
                elif pn.endswith('gamma'):
                    no_decay.add(full_param_name)

        param_dict = {pn: p for pn, p in self.named_parameters()}
        inter_params = decay & no_decay
        union_params = decay | no_decay

        assert len(inter_params) == 0, f"参数同时存在于 decay 和 no_decay: {inter_params}"
        
        unused_params = param_dict.keys() - union_params
        if len(unused_params) > 0:
            no_decay.update(unused_params)

        optim_groups = [
            {"params": [param_dict[pn] for pn in sorted(list(decay))], "weight_decay": train_config.weight_decay},
            {"params": [param_dict[pn] for pn in sorted(list(no_decay))], "weight_decay": 0.0},
        ]

        optimizer = torch.optim.AdamW(optim_groups, lr=train_config.lr, betas=train_config.betas)
        return optimizer

    def forward(self, idx, targets=None):
        device = idx.device
        bs, seq_len = idx.shape
        assert seq_len <= self.block_size, f"序列长度 {seq_len} 超过 block_size {self.block_size}"
        
        pos = torch.arange(0, seq_len, device=device, dtype=torch.long).unsqueeze(0)
        token_emb = self.transformer.wte(idx)
        position_emb = self.transformer.wpe(pos)
        x = token_emb + position_emb
        
        for block in self.transformer.h:
            x = block(x)
        x = self.transformer.norm_f(x)
        logits = self.lm_head(x)

        loss = None
        if targets is not None:
            loss = F.cross_entropy(
                logits.view(-1, logits.size(-1)),
                targets.view(-1),
                ignore_index=-1
            )
        return logits, loss

    @torch.no_grad()
    def generate(self, idx, max_new_tokens, temperature=1.0, top_k=None, do_sample=False, eos_token_id=50256):
        """生成文本
        
        Args:
            idx: 输入 token ids, shape (bs, seq_len)
            max_new_tokens: 最大生成 token 数
            temperature: 采样温度
            top_k: top-k 采样
            do_sample: 是否采样，False 则使用贪心
            eos_token_id: 结束 token id
        """
        bs = idx.shape[0]
        finished = torch.zeros(bs, device=idx.device, dtype=torch.bool)
        
        # 将 eos_token_id 转为 tensor
        eos_tensor = torch.tensor(eos_token_id, device=idx.device, dtype=torch.long)

        for _ in range(max_new_tokens):
            # 只取最后 block_size 个 token
            idx_cond = idx if idx.size(1) <= self.block_size else idx[:, -self.block_size:]
            logits, _ = self.forward(idx_cond)
            logits = logits[:, -1, :] / temperature

            if top_k is not None:
                v, _ = torch.topk(logits, k=min(top_k, logits.size(-1)))
                logits[logits < v[:, [-1]]] = -float('inf')

            probs = F.softmax(logits, dim=-1)
            if do_sample:
                idx_next = torch.multinomial(probs, num_samples=1)
            else:
                _, idx_next = torch.topk(probs, k=1, dim=-1)
            
            # 对已完成序列填充 eos_token
            idx_next = torch.where(finished.unsqueeze(-1), eos_tensor, idx_next)
            idx = torch.cat((idx, idx_next), dim=1)
            
            finished = finished | (idx_next.squeeze(-1) == eos_token_id)
            if torch.all(finished):
                break
                
        return idx
